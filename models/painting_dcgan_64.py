...
class decoder_audio(nn.Module):
    ...
    def forward(self, input):
        vec = input  # , skip
        d1 = self.upc1(vec.view(-1, self.dim, 1, 1))
        d2 = self.upc2(d1)  # (torch.cat([d1, skip[3]], 1))
        d3 = self.upc3(d2)  # (torch.cat([d2, skip[2]], 1))
        d4 = self.upc4(d3)  # (torch.cat([d3, skip[1]], 1))
        output = self.upc5(d4)  # (torch.cat([d4, skip[0]], 1))
        return output
