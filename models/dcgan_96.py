...
class encoder(nn.Module):
    def __init__(self, dim, nc=1):
        ...
        self.c5 = nn.Sequential(nn.Conv2d(nf * 8, dim, 6, 1, 0), nn.BatchNorm2d(dim), nn.Tanh())
...
class decoder(nn.Module):
    def __init__(self, dim, nc=1):
        ...
        self.upc1 = nn.Sequential(
            ...
            nn.ConvTranspose2d(dim, nf * 8, 6, 1, 0),
            nn.BatchNorm2d(nf * 8),
            nn.LeakyReLU(0.2, inplace=True),
        )
        # state size. (nf*8) x 6 x 6
...
class alpha_net_spatial(nn.Module):  # Spatial Attention with Loop
    ...
    def forward(self, h, opt=None):  # h_att_list
        ...
        h = h.transpose(1, 2).transpose(2, 3)
        att_rep = h.repeat(1, 1, 1, 1)
