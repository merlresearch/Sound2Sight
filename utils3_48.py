...
def load_dataset(opt):
    if opt.dataset == "smmnist":
        from data.moving_mnist2 import MovingMNIST_STFT_Resize as MovingMNIST
        ...
        train_data = RobotPush(
            data_root=opt.data_root, train=True, seq_len=opt.n_past + opt.n_future, image_size=opt.image_width
        )
        test_data = RobotPush(data_root=opt.data_root, train=False, seq_len=opt.n_eval, image_size=opt.image_width)
    elif opt.dataset == "kth":
        from data.kth import KTH

        train_data = KTH(
            train=True, data_root=opt.data_root, seq_len=opt.n_past + opt.n_future, image_size=opt.image_width
        )
        test_data = KTH(train=False, data_root=opt.data_root, seq_len=opt.n_eval, image_size=opt.image_width)
...
def save_np_img(fname, x):
    if x.shape[0] == 1:
        x = np.tile(x, (3, 1, 1))
    x = x.swapaxes(0, 1).swapaxes(1, 2).clamp(0, 1)
    imageio.imsave(fname, x)
